...
class Star extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      height: size,
      child: CustomPaint(
        painter: _StarPainter(color?? Colors.orange),
      ),
    );
  }
...
class StarRating extends StatelessWidget {
  ...
  const StarRating({Key? key, required this.value, this.color = Colors.deepOrange, this.starSize = 25}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return Row(
      children: List.generate(value, (_) => Padding(padding: EdgeInsets.all(2.0),
